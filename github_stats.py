...
def _hash_name(name: str) -> str:
    """
    Generate a one-way hash for a name to use in cache storage.
    This prevents sensitive names from being stored in plain text.
    :param name: Name to hash (e.g., "owner/repo-name")
    :return: SHA-256 hash of the name
    """
...
class Stats(object):
    ...
    def _is_repo_cached(self, repo: str, pushed_at: str) -> bool:
        """
        ...
        return (
            repo_hash in self._repo_cache
            and repo_hash in self._repo_timestamps
            and self._repo_timestamps[repo_hash] == pushed_at
        )
    ...
    async def lines_changed(self) -> tuple[int, int]:
        ...
        for repo in repos_list:
            ...
            if self._is_repo_cached(repo, pushed_at):
                ...
                print(
                    f"Using cached data for {repo_hash[:8]} "
                ...
                self._repo_cache.setdefault(repo_hash, {})
                self._repo_cache[repo_hash]["additions"] = repo_additions
                self._repo_cache[repo_hash]["deletions"] = repo_deletions
                self._repo_timestamps[repo_hash] = pushed_at
    ...
    async def views(self) -> int:
        ...
        for repo in repos_list:
            ...
            if self._is_repo_cached(repo, pushed_at):
                ...
                total += cached_views
                print(
                    f"Using cached views for {repo_hash[:8]} "
                    f"({processed_count}/{total_repos})"
                )
            else:
                # Fetch fresh data from API
                ...
                # Cache the result for this repo
                self._repo_cache.setdefault(repo_hash, {})
                self._repo_cache[repo_hash]["views"] = repo_views
                self._repo_timestamps[repo_hash] = pushed_at
